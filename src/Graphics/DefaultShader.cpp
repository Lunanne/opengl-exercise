...
DefaultShader::DefaultShader(const char *p_vsFilePath, const char *p_fsFilePath, const Camera camera, const Light light)
    ...
    printf("camera %f %f %f %f \n", camera.m_viewAngle,std::atan2(1.f, 2.f * 1.57f), camera.m_nearDistance, camera.m_farDistance);
    glm::mat4 projectionMatrix = glm::perspective(1.57f, camera.m_viewRatio, camera.m_nearDistance,
